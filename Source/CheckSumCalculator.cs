...
namespace S7Patcher.Source
{
    internal class CheckSumCalculator
    {
        ...
        private uint? UpdatePEHeaderFileCheckSum(string Path, long Size)
        {
            ...
            if (Result == 0x0)
            {
                Helpers.Instance.WriteWrapper(ConsoleColorType.ERROR, "CheckSumMappedFile failed with error code: " + Result);
                return null;
            }
        ...
        private bool WritePEHeaderFileCheckSum(FileStream Stream, byte[] CheckSum)
        {
            ...
            {
                Helpers.Instance.WriteWrapper(ConsoleColorType.ERROR, "CheckSum offset not found! Skipping ...");
                return false;
            }
